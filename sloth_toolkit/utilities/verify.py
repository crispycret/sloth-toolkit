...
def is_type_of(obj, cls, ignore=False):
	...
	if isinstance(obj, cls):
		...
		return False
	raise TypeError('%r is not of type %r' % (probe.get_class(obj), cls))
...
def has_attr_of_type(obj, attrname, cls, ignore=False): 
	...
	if hasattr(obj, attrname):
		...
		raise TypeError('%s is of type %r, expected type %r' %\
			('%s.%s' % (probe.get_class_name(obj), attrname), probe.get_class(attr), cls))
	raise AttributeError('%r has no attribute `%s`' % (probe.get_class(obj), attrname))
